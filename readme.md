...
* AWS에 대한 전반적인 소개(30분)
    ...
    * 리전 - aws가 서비스하는 지리적으로 인프라를 추상화 한 개념, 2개 이상의 가용영역으로 이루어져 있음
    ...
    * 고가용성(High Availability), 내결함성(Fault Tolerant)의 의미(내결함성이 언급되지 않는다면 이 부분은 빼도 좋다)
        * 고가용성
            * 중요한 이유는 인터넷이 24/7로 열려 있고 모든 시간대에 사용자들이 있기 때문이다.
            * 서버와 네트워크, 프로그램 등의 정보 시스템이 상당히 오랜 기간 동안 지속적으로 정상 운영이 가능한 성질. 절대 고장나지 않음을 의미.
            * 한 리전 내의 여러 AZ에 컴퓨팅 부하를 분산시키면 서비스 중단 '영향 범위(blast radius)'가 줄어든다. 
